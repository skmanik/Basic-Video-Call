...
export default class RTCClient {
  ...
  handleEvents() {
    ...
    this._client.on('stream-subscribed', (evt) => {
      ...
      remoteStream.play('remote_video_' + id, {fit: 'cover'}, error => {
        if (error) {
          console.log('Stream.play() Error with playback, id:', id, error)
        }
      })
        ...
        this._client.join(data.token ? data.token : null, data.channel, data.uid ? +data.uid : null, (uid) => {
          ...
          this._localStream.init(() => {
            ...
            this._localStream.play('local_stream', {fit: 'cover'}, error => {
              if (error) {
                console.log('Stream.play() Error with playback, localStream', error)
              }
            })
  ...
  showVideo () {
    ...
    console.log('showing video', this._localStream)
